...
import { IPageProducto, IProducto, IProducto2Send } from '../interfaces/productos';
...
export class ProductosService {
  ...
  getAll(): Observable<IProducto> {
    return this.http.get<IProducto>(this.sURL + "/all", httpOptions);
  }
  ...
  getPage(rpp: number, page: number, filter: string, order: string, direction: string, tipoproducto: number): Observable<IPageProducto> {
    ...
    return this.http.get<IPageProducto>(this.sURL +"/page", httpOptions);
  }
